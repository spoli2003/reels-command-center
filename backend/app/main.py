...
from app.api.synchronization import router as synchronization_router
from app.core.config import get_settings
from app.db.session import SessionLocal
from app.services import meta_scheduler, youtube_scheduler
from app.services.sync_recovery import recover_interrupted_sync_runs
...
async def lifespan(_app: FastAPI):
    db = SessionLocal()
    try:
        recover_interrupted_sync_runs(db)
    finally:
        db.close()
...
app = FastAPI(title=settings.app_name, version="0.8.4-unified-platform-experience", lifespan=lifespan)
