...
import type { PlatformKey, PlatformTopicSummary } from "../lib/platform-api";
...
/** Mirrors components/topic-card.tsx exactly, linking into the generic
 * /platforms/{platform}/videos/{id} surface instead of /youtube/videos/{id}. */
export function PlatformTopicCard({ platform, topic }: { platform: PlatformKey; topic: PlatformTopicSummary }) {
  const sameVideo = topic.worst_video && topic.best_video && topic.worst_video.external_id === topic.best_video.external_id;
  ...
  return (
    <article className="topicCard">
      <div className="topicCardHeader">
        <h4>„{topic.keyword}”</h4>
        <span className="topicCardCount">{topic.video_count} materiałów</span>
      </div>
      ...
      <div className="topicCardVideos">
        {topic.best_video ? (
          <Link href={`/platforms/${platform}/videos/${topic.best_video.external_id}`} title={topic.best_video.title}>
            🏆 Najlepszy: {truncateTitle(topic.best_video.title, 32)}
          </Link>
        ) : null}
        {topic.worst_video && !sameVideo ? (
          <Link href={`/platforms/${platform}/videos/${topic.worst_video.external_id}`} title={topic.worst_video.title}>
