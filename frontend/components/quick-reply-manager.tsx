...
import type { QuickReplyTemplate } from "../lib/youtube-api";

const API_URL = process.env.NEXT_PUBLIC_API_URL ?? "http://127.0.0.1:8000";

/** Part 9 — create/edit/delete locally managed quick-reply templates. Selecting a
 * template into the composer never sends it; only this manager mutates templates. */
export function QuickReplyManager({
  ...
  async function create() {
    ...
    setError("");
    try {
      const response = await fetch(`${API_URL}/api/integrations/youtube/quick-replies`, {
        method: "POST",
  ...
  async function save(id: number) {
    ...
    setError("");
    try {
      const response = await fetch(`${API_URL}/api/integrations/youtube/quick-replies/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text }),
      });
  ...
  async function remove(id: number) {
    ...
    setError("");
    try {
      const response = await fetch(`${API_URL}/api/integrations/youtube/quick-replies/${id}`, { method: "DELETE" });
